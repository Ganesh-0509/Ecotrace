...
const REQUIRED_KEYS = ['title', 'detail', 'impact', 'category'] as const;
...
describe('generateRuleInsights', () => {
  ...
  it('is deterministic for the same input', () => {
    const profile = { transport: 'car', diet: 'meat_heavy', energy: 'grid' } as const;
  ...
  it('de-prioritises a category the user has already worked on', () => {
    ...
    const profile = { transport: 'transit', diet: 'balanced', energy: 'mixed' } as const;
    const busy = generateRuleInsights(profile, {
      entries: Array.from(
        { length: 20 },
        () => ({ category: 'transport', co2SavedKg: 3 }) as unknown as LogEntry
      ),
    });
