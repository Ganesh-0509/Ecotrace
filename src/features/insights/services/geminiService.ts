...
import type { InsightSummary } from '../prompts/insightPrompt';
import { ACTION_INDEX } from '../../../utils/emissionFactors';
import type { Insight, MonthLog, UserProfile } from '../../../domain/models';
...
function summarise(monthLog: Partial<MonthLog> | null | undefined): InsightSummary {
...
export async function generateInsights(
  ...
  monthLog: Partial<MonthLog> | null | undefined
): Promise<Insight[]> {
  const ai = getClient();
  ...
  const list: Insight[] = Array.isArray(parsed)
    ? (parsed as Insight[])
    : ((parsed as { recommendations?: Insight[] })?.recommendations ?? []);
