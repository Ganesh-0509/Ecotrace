...
/** Compact activity summary the prompt builder turns into the model payload. */
export interface InsightSummary {
  count: number;
  totalSaved: number;
  byCategory: Record<string, number>;
  topActions: Array<{ action: string; times: number }>;
}
 ...
 */
export function buildInsightPrompt(profile: Partial<UserProfile> | null, summary: InsightSummary) {
  const payload = {
    profile: {
